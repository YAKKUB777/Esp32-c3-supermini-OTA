/*
 * RF TOOL v4.0 - Bruce Style UI for ESP32-C3 Supermini
 * 
 * Features:
 * - Wi-Fi Scanner & Deauther
 * - Sub-GHz Scanner & Jammer (CC1101 via RadioLib)
 * - Settings (frequency selection)
 * 
 * Pinout:
...
uint8_t freqIndex = 1; // 433.92 за замовчуванням
...
void drawWiFiScanList() {
  ...
  for (int i = 0; i < apCount; i++) {
    int y = 30 + i * 15;
    if (y > 130) break; // вміщається ~7 рядків
...
void drawSubGHzScanScreen() {
  ...
  drawButtonHints("SEL:start/stop  L/R:change freq  BACK:back");
}
...
void handleNavigation() {
  ...
  if (millis() - lastPress < 180) return; // дебаунс
  ...
  // Універсальна обробка BACK
  if (back) {
    if (currentState == MAIN_MENU) return;
    
    // Зупиняємо активні процеси
  ...
  switch (currentState) {
    ...
    case WIFI_MENU:
      ...
      if (sel) {
        if (wifiMenuIndex == 0) { // Scan
          startWiFiScan();
          currentState = WIFI_SCAN;
        } else if (wifiMenuIndex == 1) { // Deauth
          if (apCount == 0) {
            startWiFiScan();
          }
          currentState = WIFI_DEAUTH;
          drawWiFiDeauthScreen();
        } else if (wifiMenuIndex == 2) { // Back
          currentState = MAIN_MENU;
          drawMainMenu();
        }
      }
